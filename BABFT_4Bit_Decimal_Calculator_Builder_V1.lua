...
title.Text="4비트 10진수 계산기 자동건설 V1"
...
info.Text="A/B 4비트 · SUB OFF=덧셈 / ON=뺄셈\n결과 -15~30 · 10진수 두 자리 + 음수 표시\nGate 약 145 · Switch 9 · Display 75"
...
local function build()
 ...
 if not bt or not pt or not bind then return fail("필요한 도구가 없습니다.") end
 ...
 local function sw(n,x)
  specs[#specs+1]={n=n,k="Switch",id=SWITCH_ID,cf=BASE*CFrame.new(x,0,0)}
  return n
 end
 local function gate(n,t)
  ...
  specs[#specs+1]={n=n,k="Gate",id=GATE_ID,t=t,cf=BASE*CFrame.new(2+col*3,-5-row*2.3,-8)}
  return n
 end
 ...
 for c=1,5 do out(sign,disp("MINUS_"..c,34+(c-1)*2,20)) end
